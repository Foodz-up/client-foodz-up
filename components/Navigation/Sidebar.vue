          ...
          <div v-if="isConnected" class="flex items-center">
            <button class="relative inline-block" @mouseover="cartToggle()" @mouseleave="cartToggle()">
              <nuxt-link to="/panier">
                ...
                <span v-if="oneItemOrMore" class="absolute top-0 right-0 inline-flex items-center justify-center p-1 text-xs font-bold leading-none text-red-100 transform -translate-x-4 -translate-y-1/4  bg-red-600 rounded-full">
                  {{ numberItemsInCart }}
                </span>
              </nuxt-link>
            ...
            <nuxt-link to="/profil">
              <!-- <img class="h-10 w-10 rounded-full ring-2 ring-white" src="https://images.unsplash.com/photo-1491528323818-fdd1faba62cc?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt=""> -->
              <div>
    ...
    <div :class="{ hidden: !phoneMenu }" class="absolute top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
      <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
        <div class="pt-5 pb-6 px-5">
          ...
          <nuxt-link to="/panier" class="flex items-center mt-6">
            <DynamicSvg :width="20" class="text-primary" :component-name="'cart'" />
            <span class="ml-3 text-base font-medium text-gray-900">
              Panier
            </span>
          </nuxt-link>
...
export default class Sidebar extends Vue {
      ...
      get oneItemOrMore ():boolean {
        return this.numberItemsInCart > 0
      }
}
</script>
